...
// FollowAcrossLive must skip a session's history from before the follow
// began (root, present at the first tick) while still delivering a session
// that only appears afterward (late) in full — including its own first
// event, appended essentially atomically with the session becoming known.
func TestFollowAcrossLive_SkipsPriorHistoryButNotANewlyAppearingSession(t *testing.T) {
	s := NewStore(t.TempDir())
	if _, _, _, err := s.Append(event.Event{SessionName: "root", Type: "t", Direction: event.Internal, Summary: "before"}); err != nil {
		t.Fatal(err)
	}
	...
	// Lets the first tick prime "root" before anything else happens.
	time.Sleep(50 * time.Millisecond)

	if _, _, _, err := s.Append(event.Event{SessionName: "root", Type: "t", Direction: event.Internal, Summary: "after"}); err != nil {
		t.Fatal(err)
	}
	mu.Lock()
	names = append(names, "late")
	mu.Unlock()
	if _, _, _, err := s.Append(event.Event{SessionName: "late", Type: "t", Direction: event.Internal, Summary: "late-first"}); err != nil {
		t.Fatal(err)
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("timed out waiting for both events, got %d", n)
		}
	...
	if want := []string{"after", "late-first"}; !slices.Equal(summaries, want) {
		t.Fatalf("delivered summaries = %v, want %v (root's \"before\" must not replay)", summaries, want)
	}
}
