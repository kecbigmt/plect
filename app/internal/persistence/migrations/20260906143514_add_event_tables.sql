...
-- create index "event_streams_session_name_created_at" to table: "event_streams"
CREATE INDEX `event_streams_session_name_created_at` ON `event_streams` (`session_name`, `created_at` DESC);
-- create "events" table
CREATE TABLE `events` (`id` text NULL, `stream_id` text NOT NULL, `sequence` integer NOT NULL, `time` text NOT NULL, `type` text NOT NULL, `source` text NOT NULL, `direction` text NOT NULL, `summary` text NOT NULL, `body` text NOT NULL DEFAULT '', `metadata_json` text NOT NULL, `delivery_mode` text NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `0` FOREIGN KEY (`stream_id`) REFERENCES `event_streams` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION, CHECK (sequence > 0), CHECK (direction IN ('inbound', 'outbound', 'internal')), CHECK (json_valid(metadata_json)));
...
CREATE TABLE `event_cursors` (`stream_id` text NOT NULL, `kind` text NOT NULL, `next_sequence` integer NOT NULL, PRIMARY KEY (`stream_id`, `kind`), CONSTRAINT `0` FOREIGN KEY (`stream_id`) REFERENCES `event_streams` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE, CHECK (kind IN ('delivery', 'tick', 'heartbeat')), CHECK (next_sequence >= 0));
...
DROP TABLE `events`;
-- reverse: create index "event_streams_session_name_created_at" to table: "event_streams"
DROP INDEX `event_streams_session_name_created_at`;
