...
-- create "events" table
CREATE TABLE `events` (`id` text NULL, `stream_id` text NOT NULL, `sequence` integer NOT NULL, `time` text NOT NULL, `type` text NOT NULL, `source` text NOT NULL, `direction` text NOT NULL, `summary` text NOT NULL, `body` text NOT NULL DEFAULT '', `metadata_json` text NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `0` FOREIGN KEY (`stream_id`) REFERENCES `event_streams` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION, CHECK (sequence > 0), CHECK (direction IN ('inbound', 'outbound', 'internal')));
