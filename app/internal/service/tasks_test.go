...
func TestUp_VanishedTmuxPaneIsRebuilt(t *testing.T) {
	...
	cfg := writeWorkflowFixture(t, t.TempDir(), "default",
		[]taskFixture{
			{id: "tmux", scope: "run", setup: "touch " + pane + "; echo '{}'", alive: "test -f " + pane},
		},
		[]nodeFixture{{id: "tmux"}},
	)
	seedSession(t, store, sessionName, "org/repo", 12, "default", map[string]*contract.TaskState{
	...
	if st := result.Tasks["tmux"]; st == nil || st.Status != contract.TaskStatusProduced {
		t.Fatalf("tmux = %+v, want rebuilt and produced", st)
	}
...
func TestUp_VanishedGuardDirectoryIsRebuilt(t *testing.T) {
	...
	seedSession(t, store, sessionName, "org/repo", 12, "default", map[string]*contract.TaskState{
		"gh_guard": {Scope: contract.TaskScopeSession, Status: contract.TaskStatusProduced, Outputs: map[string]any{"dir": filepath.Join(t.TempDir(), "gone")}},
		"claude":   {Scope: contract.TaskScopeRun, Status: contract.TaskStatusProduced, Outputs: map[string]any{"saw": "stale"}},
	})
	...
	dir, _ := result.Tasks["gh_guard"].Outputs["dir"].(string)
	if dir == "" {
		t.Fatalf("gh_guard = %+v, want a freshly produced dir", result.Tasks["gh_guard"])
	}
	if _, statErr := os.Stat(dir); statErr != nil {
		t.Fatalf("gh_guard's rebuilt dir does not exist: %v", statErr)
	}
	if st := result.Tasks["claude"]; st == nil || st.Outputs["saw"] != dir {
		t.Fatalf("claude did not rebuild against gh_guard's rebuilt dir: %+v", st)
	}
}
...
func TestUp_VanishedSubscriptionIsRebuilt(t *testing.T) {
	...
	cfg := writeWorkflowFixture(t, t.TempDir(), "default",
		[]taskFixture{
			{id: "slack_subscribe", scope: "run", setup: "touch " + registration + "; echo '{}'", alive: "test -f " + registration},
		},
		[]nodeFixture{{id: "slack_subscribe"}},
	)
	seedSession(t, store, sessionName, "org/repo", 12, "default", map[string]*contract.TaskState{
		"slack_subscribe": {Scope: contract.TaskScopeRun, Status: contract.TaskStatusProduced, Outputs: map[string]any{}},
	})
	...
	if st := result.Tasks["slack_subscribe"]; st == nil || st.Status != contract.TaskStatusProduced {
		t.Fatalf("slack_subscribe = %+v, want rebuilt and produced", st)
	}
