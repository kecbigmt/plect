...
const (
	...
	// log when an inbound event lands on a session that is down (not
	// destroyed): no per-session reactor drains a down session's own log, so
	// without this push such an event would sit unreacted until the session is
	// brought back up. It reuses the terminal signals' single-hop-push shape
	// (D1-D3: SessionName is the receiver, MetaOriginSession the emitter) but
	// is not itself a terminal signal — the origin session is not finished,
	// and its own tick resumes handling its log the moment it comes back up.
