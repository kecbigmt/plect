...
`plect.message` and `plect.message_chunk` are the runtime-neutral contract
...
once that message's visible text is non-empty. `plect.message_chunk` is an
...
| Field | Required | Meaning |
|---|---|---|
| `message_id`, `message_id_origin`, `source` | yes | As above — the same `message_id` a preceding chunk sequence used. |
...
| `final` | yes | `true` on the sequence's last chunk for this `message_id`. |
| `ordering` | no | `strict` (default) or `best_effort`. `best_effort` marks a harness whose delta stream can drop chunks, so a gap in `index` means loss, not a bug. |
...
Body is the chunk's new text (its delta).

### Rules

1. `plect.message` is always emitted, exactly once per `message_id`, once its
   text is non-empty. `plect.message_chunk` is an optional preview: a chunk
   sequence never substitutes for the `plect.message` that follows it. A
   consumer keys on `message_id` and replaces any chunk-built preview with
...
| DeepSeek | yes (`message.id`) | `(turn_index, step_index)` = prompt cycle and model call | chunk stream keyed by envelope `seq`, `block_index` for stream position | `run_id` (`SubagentRunId`), `depth` (`delegationDepth`), `source_seq` (envelope `seq`), `stop_reason` (`turn/end.reason.kind`), `model`/`provider` (`source.{provider,model}`); no `agent_role` |
