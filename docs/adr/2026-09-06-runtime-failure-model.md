...
### 5. Forced full rebuild remains the last-resort operation

#### Options

| Option | Layer | Assessment |
|---|---|---|
| Do not build another recreation mode | core | Recommended. Plain `up` repairs the observed failure, and resetting an agent while retaining its conversation thread would create divergent state. |
| Narrow `--force-recreate` and add a session-state discard flag | core | Rejected. No present consumer needs a fresh agent conversation while retaining its Slack thread and dynamic work. |
| Replace it with a scoped recreate mode | core | Rejected. It adds an unneeded operator surface and exposes Plecture's internal scope vocabulary. |
...
`--force-recreate` remains the last-resort full rebuild. It cleans and forgets
both workflow-node scopes; unconditionally cleans and reruns the workspace
provider; and clears the workspace provider outputs, `Branch`,
`WorkspaceDirPath`, `Conversation`, `Message`, dynamic task instances, health
state, tick backoff, and runtime observation state. It preserves the session
identity, resource mapping, parent relation, inputs, and event log.

The flag originates in `docker compose up --force-recreate`, but Plecture
intentionally gives it broader semantics: resetting an agent must reset its
conversation thread too, because retaining a thread whose context the fresh
agent has not seen would make the two conversations diverge.
...
it does not emit `plect.workflow_population.up`. `--force-recreate` first
makes the member run down and then produces it again, so a
population-produced session emits `plect.workflow_population.up` for that real
down-to-up transition.

This is a core decision. Prompt aliases cannot change lifecycle semantics for
API callers, MCP callers, or scripts. Plugins do not own the session task map,
and config should not carry one-off operator intent.
...
### Add a narrower recreation mode

Rejected. Plain `up` is the repair operation for a liveness-failed node. A
runtime-only forced recreation has no present consumer: resetting an agent
while retaining its Slack thread and dynamic work would retain context the new
agent has not seen. An additive session-state discard flag and
an enum-based scoped recreate mode would only make that unsupported split
selectable, while the latter also requires callers to learn Plecture's internal
scope vocabulary.
