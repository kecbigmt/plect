...
The legacy chain-attempt key's third component is not persisted; entries that
collapse to the same `(session_id, instance, chain_id)` must have the same
fingerprint.
...
sidecar or an unexpected path in `events/` stops the import and leaves every
